	...
	MYSQL_ROW row;
		...
		while(!terminar){
			...
			else if (codigo == 2) //Peticion de registro
			{
				...
				idSQL = atoi(row[0]);
				idSQL = idSQL + 1;
				sprintf(consulta, "INSERT INTO JUGADOR VALUES ('%d','%s','%s')",idSQL, username, password); //Insertamos el id, el username y el password para añadirlos a la base de datos.
				err=mysql_query (conn, consulta);
				if (err!=0) {
					printf("Error al consultar datos de la base: %u %s\n",
						   mysql_errno(conn), mysql_error(conn));
